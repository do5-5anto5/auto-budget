...
def get_connection() -> sqlite3.Connection:
    ...
    connection.execute(
        "CREATE TABLE IF NOT EXISTS business (id INTEGER PRIMARY KEY, name TEXT, phone TEXT, logo TEXT) STRICT"
    )
    return connection


def save_business(name: str = None, phone: str = None, logo: str = None) -> str:
    ...
    try:
        ...
        if exists:
            ...
            # Insert new record
            connection.execute(
                "INSERT INTO business (id, name, phone, logo) VALUES (1, ?, ?, ?)",
                (name or "", phone or "", logo or "")
            )
...
def get_business() -> dict:
    ...
    try:
        connection = get_connection()
        cursor = connection.execute("SELECT name, phone, logo FROM business WHERE id = 1")
        row = cursor.fetchone()
...
def update_business(name: str = None, phone: str = None, logo: str = None) -> str:
    """Update specific business fields."""
    return save_business(name, phone, logo)
